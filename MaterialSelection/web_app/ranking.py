...
def score_material(material: dict, requirements: dict) -> float:
    ...
    for req_input, req_expected_val in requirements.items():
        ...
        # Scoring Logic: Proximity
        target_score = sanitize_score(req_expected_val)
        if target_score == 0: target_score = 100.0 # Default to High if unknown
        
        # Determine "Direction"
        # If target >= 50, we assume "Higher is Better" (High/Medium)
        # If target < 50, we assume "Lower is Better" (Low)
        
        match_quality = 0.0
        
        if target_score >= 50:
            # Higher is better
            if found_val >= target_score:
                # Reward exceeding: 100 + surplus
                match_quality = 100.0 + (found_val - target_score)
            else:
                # Penalize only shortfall
                match_quality = 100.0 - (target_score - found_val)
        else:
            # Lower is better (target = 25 e.g.)
            if found_val <= target_score:
                # Reward going lower: 100 + surplus
                match_quality = 100.0 + (target_score - found_val)
            else:
                # Penalize excess
                match_quality = 100.0 - (found_val - target_score)
        
        score += match_quality * w
